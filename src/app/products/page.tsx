  ...
  return (
    <div className="relative text-white pt-12 space-y-10">
      ...
      <div className="text-center space-y-3">
        ...
        {searchParams.get("searchWord") && (
          <p className="text-sm text-gray-500">
            Results for &ldquo;<span className="text-white">{searchParams.get("searchWord")}</span>&rdquo;
          </p>
        )}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 text-red-400">Failed to load products.</div>
      ) : !products?.length ? (
        <div className="flex flex-col items-center gap-4 py-24 text-gray-500">
          ...
          {hasActiveFilters && (
            <p className="text-sm text-gray-600">Try adjusting the filters in the header.</p>
          )}
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">

  ...
  return (
    <Link href={`/events/${event.slug}`}
      ...
      <div className="relative h-48 overflow-hidden bg-white/5">
        ...
        {/* Date badge */}
        <div className="absolute top-3 left-3 bg-black/70 backdrop-blur-sm border border-white/10
          rounded-xl px-3 py-2 text-center min-w-[52px]">
        ...
        {/* Status badge */}
        {isPast && (
          <div className="absolute top-3 right-3 bg-gray-800/80 text-gray-400 text-xs
            font-semibold px-2 py-1 rounded-lg backdrop-blur-sm">
            Past
          </div>
        )}
        {!isPast && event.stock === 0 && (
          <div className="absolute top-3 right-3 bg-red-900/80 text-red-300 text-xs
            font-semibold px-2 py-1 rounded-lg backdrop-blur-sm">
            Sold Out
          </div>
        )}
      ...
      <div className="flex flex-col flex-1 p-5 gap-3">
        <span className="self-start text-[10px] font-bold tracking-widest uppercase
          px-2 py-0.5 rounded-full bg-purple-500/15 text-purple-400 border border-purple-500/20">
          {event.category}
        </span>

        <h3 className="font-bold text-white text-base leading-snug group-hover:text-purple-300 transition line-clamp-2">
          {event.title}
        </h3>
        ...
        <div className="flex items-center justify-between pt-2">
          <span className="font-bold text-white text-sm">
            {isFree ? <span className="text-green-400">Free</span> : `฿${event.price.toLocaleString()}`}
          </span>
    ...
    <main className="min-h-screen text-white">
      ...
      <div className="relative overflow-hidden py-24 px-8 text-center">
        <div className="absolute inset-0 -z-10
          bg-[radial-gradient(ellipse_80%_60%_at_50%_0%,rgba(139,92,246,0.18),transparent)]" />
        <div className="absolute inset-0 -z-10
          bg-[linear-gradient(to_right,rgba(139,92,246,0.04)_1px,transparent_1px),linear-gradient(to_bottom,rgba(139,92,246,0.04)_1px,transparent_1px)]
          bg-[size:48px_48px]" />
        <p className="text-xs font-bold tracking-[0.3em] text-purple-400 uppercase mb-4">Events &amp; Tickets</p>
        <h1 className="text-5xl md:text-6xl font-extrabold leading-tight mb-4
          bg-gradient-to-r from-fuchsia-400 via-purple-300 to-cyan-400 bg-clip-text text-transparent">
          Gaming Events
        </h1>
        <p className="text-gray-400 text-lg max-w-xl mx-auto">
          Expos, tournaments, showcases — grab your ticket before they sell out.
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 pb-24 space-y-16">
        ...
        {isError && (
          <div className="text-center py-20 text-red-400">Failed to load events.</div>
        )}
        ...
        {upcoming.length > 0 && (
          <section>
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="ml-1 text-xs bg-purple-500/20 text-purple-300 px-2 py-0.5 rounded-full font-semibold">
                {upcoming.length}
              </span>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {!isLoading && !isError && data?.length === 0 && (
          <div className="text-center py-20 text-gray-500">No events available right now. Check back soon!</div>
        )}
      </div>
    </main>
  )
